...
double parse_value_with_metric_prefix_util(const string& val_str_orig) {
    ...
    double base_val;
    try { base_val = stod(val_str, &suffix_start_pos); }
    catch (const exception&) { throw invalid_argument("Invalid numeric value in string: \"" + val_str_orig + "\""); }
    ...
    if (suffix_str.rfind("meg", 0) == 0) multiplier = 1e6;
        ...
        switch (prefix_char) {
            case 'p': multiplier = 1e-12; break; case 'n': multiplier = 1e-9; break;
            case 'u': multiplier = 1e-6; break; case 'm': multiplier = 1e-3; break;
            case 'k': multiplier = 1e3; break; case 'g': multiplier = 1e9; break;
        }
    }
...
vector<double> gaussian_elimination_matrix(vector<vector<double>> A, vector<double> b) {
    ...
    for (int i = 0; i < n; ++i) {
        int max_row = i;
        for (int k = i + 1; k < n; ++k) if (abs(A[k][i]) > abs(A[max_row][i])) max_row = k;
        swap(A[i], A[max_row]);
        swap(b[i], b[max_row]);
        for (int k = i + 1; k < n; ++k) {
...
public:
    ...
    virtual void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m_map, double h, const vector<double>& prev_sol) = 0;
...
public:
    Resistor(const string& name, const string& n1, const string& n2, const string& val) : Component(name, n1, n2, parse_value_with_metric_prefix_util(val)) {}
    string to_netlist_string() const override;
    void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) override;
};

class Capacitor : public Component {
public:
    Capacitor(const string& name, const string& n1, const string& n2, const string& val) : Component(name, n1, n2, parse_value_with_metric_prefix_util(val)) {}
    string to_netlist_string() const override;
    void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) override;
};
...
public:
    Inductor(const string& name, const string& n1, const string& n2, const string& val) : Component(name, n1, n2, parse_value_with_metric_prefix_util(val)) {}
    string to_netlist_string() const override;
    void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) override;
};
...
public:
    CurrentSource(const string& name, const string& n1, const string& n2, const string& val) : Component(name, n1, n2, parse_value_with_metric_prefix_util(val)) {}
    string to_netlist_string() const override;
    void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) override;
};
...
public:
    string ctrl_node1, ctrl_node2;
    VCVS(const string& name, const string& n1, const string& n2, const string& cn1, const string& cn2, const string& gain_str)
            : Component(name, n1, n2, parse_value_with_metric_prefix_util(gain_str)), ctrl_node1(cn1), ctrl_node2(cn2) {}
    string to_netlist_string() const override;
    void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) override;
};
...
public:
    string ctrl_node1, ctrl_node2;
    VCCS(const string& name, const string& n1, const string& n2, const string& cn1, const string& cn2, const string& gain_str)
            : Component(name, n1, n2, parse_value_with_metric_prefix_util(gain_str)), ctrl_node1(cn1), ctrl_node2(cn2) {}
    string to_netlist_string() const override;
    void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) override;
};
...
public:
    string ctrl_v_name;
    CCVS(const string& name, const string& n1, const string& n2, const string& cvn, const string& gain_str)
            : Component(name, n1, n2, parse_value_with_metric_prefix_util(gain_str)), ctrl_v_name(cvn) {}
    string to_netlist_string() const override;
    void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) override;
};
...
public:
    string ctrl_v_name;
    CCCS(const string& name, const string& n1, const string& n2, const string& cvn, const string& gain_str)
            : Component(name, n1, n2, parse_value_with_metric_prefix_util(gain_str)), ctrl_v_name(cvn) {}
    string to_netlist_string() const override;
    void stamp(Circuit& circuit, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) override;
};
...
void Circuit::clear() {
    ...
    voltage_source_list.clear(); inductor_list.clear(); vcvs_list.clear(); ccvs_list.clear();
    ground_node_exists = true; ground_node_explicit_name = "0";
    tran_solved = false; tran_results.clear();
    dc_sweep_solved = false; dc_sweep_results.clear(); dc_sweep_source_name = "";
}
...
int Circuit::prepare_for_analysis() {
    node_to_idx.clear(); idx_to_node_name.clear();
    voltage_source_list.clear(); inductor_list.clear(); vcvs_list.clear(); ccvs_list.clear();
...
void Circuit::build_mna_matrix(vector<vector<double>>& A, vector<double>& z, double h, const vector<double>& prev_sol) {
    ...
    int system_size = N + M;
    if (system_size == 0) { A.clear(); z.clear(); return; }
...
void Circuit::perform_transient_analysis(double t_step, double t_stop) {
    ...
    int N = prepare_for_analysis();
    int M = voltage_source_list.size() + inductor_list.size() + vcvs_list.size() + ccvs_list.size();
    if (N + M == 0) { tran_solved = true; return; }
...
void Circuit::perform_dc_sweep_analysis(const string& src, double start, double end, double inc) {
    ...
    int N = prepare_for_analysis();
    int M = voltage_source_list.size() + inductor_list.size() + vcvs_list.size() + ccvs_list.size();
    if (N + M == 0) { dc_sweep_solved = true; return; }
    ...
    sweep_comp->value = orig_val; // Restore original value
...
void Resistor::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
    double g = 1.0 / value; int i1 = c.get_node_matrix_index(node1_name); int i2 = c.get_node_matrix_index(node2_name);
    if (i1 >= 0) G[i1][i1] += g; if (i2 >= 0) G[i2][i2] += g;
    if (i1 >= 0 && i2 >= 0) { G[i1][i2] -= g; G[i2][i1] -= g; }
}

string Capacitor::to_netlist_string() const { ostringstream oss; oss << name << " " << node1_name << " " << node2_name << " " << value; return oss.str(); }
void Capacitor::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
    if (h == 0) return;
    double g_eq = value / h;
    int i1 = c.get_node_matrix_index(node1_name); int i2 = c.get_node_matrix_index(node2_name);
    if (i1 >= 0) G[i1][i1] += g_eq; if (i2 >= 0) G[i2][i2] += g_eq;
    if (i1 >= 0 && i2 >= 0) { G[i1][i2] -= g_eq; G[i2][i1] -= g_eq; }
    if (!p.empty()) {
        double v1_prev = (i1 >= 0) ? p[i1] : 0.0; double v2_prev = (i2 >= 0) ? p[i2] : 0.0;
        double i_eq = g_eq * (v1_prev - v2_prev);
        if (i1 >= 0) J[i1] += i_eq; if (i2 >= 0) J[i2] -= i_eq;
    }
}

string Inductor::to_netlist_string() const { ostringstream oss; oss << name << " " << node1_name << " " << node2_name << " " << value; return oss.str(); }
void Inductor::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
    int m_idx = m.at(name);
    int i1 = c.get_node_matrix_index(node1_name); int i2 = c.get_node_matrix_index(node2_name);
    if (i1 >= 0) { B[i1][m_idx] += 1.0; C[m_idx][i1] += 1.0; }
    if (i2 >= 0) { B[i2][m_idx] -= 1.0; C[m_idx][i2] -= 1.0; }
...
VoltageSource::VoltageSource(const string& v_name, const string& n1, const string& n2, const vector<string>& params)
        : Component(v_name, n1, n2, 0.0), sourceType(DC), dc_offset(0), amplitude(0), frequency(0), v1(0), v2(0), td(0), tr(0), tf(0), pw(0), per(0), raw_params(params) {
    ...
    if (first_param_upper.rfind("sin", 0) == 0) {
        string combined_params; for (size_t i = 0; i < params.size(); ++i) { combined_params += params[i] + " "; }
        ...
        stringstream ss(sin_params_str); string offset_str, amp_str, freq_str; ss >> offset_str >> amp_str >> freq_str;
        if (ss.fail() || !ss.eof()) throw runtime_error("Invalid SIN() params for " + name);
        sourceType = SINUSOIDAL; dc_offset = parse_value_with_metric_prefix_util(offset_str);
        amplitude = parse_value_with_metric_prefix_util(amp_str); frequency = parse_value_with_metric_prefix_util(freq_str);
        ...
        stringstream ss(pulse_params_str); string v1_s, v2_s, td_s, tr_s, tf_s, pw_s, per_s;
        ss >> v1_s >> v2_s >> td_s >> tr_s >> tf_s >> pw_s >> per_s;
        if (ss.fail() || !ss.eof()) throw runtime_error("Invalid PULSE() params for " + name);
        sourceType = PULSE; v1 = parse_value_with_metric_prefix_util(v1_s); v2 = parse_value_with_metric_prefix_util(v2_s);
        td = parse_value_with_metric_prefix_util(td_s); tr = parse_value_with_metric_prefix_util(tr_s);
        tf = parse_value_with_metric_prefix_util(tf_s); pw = parse_value_with_metric_prefix_util(pw_s);
        per = parse_value_with_metric_prefix_util(per_s); this->value = v1;
    } else if (params.size() == 1) {
        sourceType = DC; this->value = parse_value_with_metric_prefix_util(params[0]);
...
void VoltageSource::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
    int m_idx = m.at(name); int i1 = c.get_node_matrix_index(node1_name); int i2 = c.get_node_matrix_index(node2_name);
    if (i1 >= 0) { B[i1][m_idx] += 1.0; C[m_idx][i1] += 1.0; }
    if (i2 >= 0) { B[i2][m_idx] -= 1.0; C[m_idx][i2] -= 1.0; }
    E[m_idx] += this->value;
}
...
string CurrentSource::to_netlist_string() const { ostringstream oss; oss << name << " " << node1_name << " " << node2_name << " " << value; return oss.str(); }
void CurrentSource::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
    int i1 = c.get_node_matrix_index(node1_name); int i2 = c.get_node_matrix_index(node2_name);
    if (i1 >= 0) J[i1] -= value; if (i2 >= 0) J[i2] += value;
}

string VCVS::to_netlist_string() const { ostringstream oss; oss << name << " " << node1_name << " " << node2_name << " " << ctrl_node1 << " " << ctrl_node2 << " " << value; return oss.str(); }
void VCVS::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
    int m_idx = m.at(name); int n1_idx = c.get_node_matrix_index(node1_name); int n2_idx = c.get_node_matrix_index(node2_name);
    int cn1_idx = c.get_node_matrix_index(ctrl_node1); int cn2_idx = c.get_node_matrix_index(ctrl_node2);
...
string VCCS::to_netlist_string() const { ostringstream oss; oss << name << " " << node1_name << " " << node2_name << " " << ctrl_node1 << " " << ctrl_node2 << " " << value; return oss.str(); }
void VCCS::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
    int n1_idx = c.get_node_matrix_index(node1_name); int n2_idx = c.get_node_matrix_index(node2_name);
    int cn1_idx = c.get_node_matrix_index(ctrl_node1); int cn2_idx = c.get_node_matrix_index(ctrl_node2);
...
string CCVS::to_netlist_string() const { ostringstream oss; oss << name << " " << node1_name << " " << node2_name << " " << ctrl_v_name << " " << value; return oss.str(); }
void CCVS::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
...
string CCCS::to_netlist_string() const { ostringstream oss; oss << name << " " << node1_name << " " << node2_name << " " << ctrl_v_name << " " << value; return oss.str(); }
void CCCS::stamp(Circuit& c, vector<vector<double>>& G, vector<vector<double>>& B, vector<vector<double>>& C, vector<vector<double>>& D, vector<double>& J, vector<double>& E, map<string, int>& m, double h, const vector<double>& p) {
    if (m.find(ctrl_v_name) == m.end()) throw runtime_error("Control source " + ctrl_v_name + " not found for " + name);
    int m_idx_ctrl = m.at(ctrl_v_name);
    int n1_idx = c.get_node_matrix_index(node1_name);
    int n2_idx = c.get_node_matrix_index(node2_name);
...
struct Command { string type; vector<string> args; };
...
Command Parser::parse_line(const string& line) {
    stringstream ss(trim_string_util(line)); string segment; Command cmd; bool first = true;
    while (ss >> segment) { if (first) { cmd.type = segment; first = false; } else { cmd.args.push_back(segment); } }
    return cmd;
}
...
void Parser::execute_command(Command& cmd, Circuit& circuit) {
    try {
        ...
        if (cmd_type_lower == "exit" || cmd_type_lower == "quit") { cout << "Exiting simulator." << endl; exit(0); }
        else if (cmd_type_lower == ".list") { handle_list_components(circuit); }
        else if (cmd_type_lower == ".nodes") { handle_list_nodes(circuit); }
        else if (cmd_type_lower == ".clear") { circuit.clear(); cout << "Circuit cleared." << endl; }
        else if (cmd_type_lower == ".print") { handle_print(cmd.args, circuit); }
        else { cmd.args.insert(cmd.args.begin(), cmd.type); handle_add_component(cmd.args, circuit); }
...
void Parser::handle_add_component(const vector<string>& args, Circuit& circuit) {
    ...
    switch(type_char) {
        ...
        default: throw runtime_error("Unknown component type: " + string(1, type_char));
    }
...
void Parser::handle_list_components(const Circuit& circuit) {
    ...
    if (circuit.components.empty()) { cout << "  (No components)" << endl; return; }
    for (const auto& comp : circuit.components) { cout << "  - " << comp->to_netlist_string() << endl; }
}
void Parser::handle_list_nodes(const Circuit& circuit) {
    set<string> all_nodes;
    for (const auto& comp : circuit.components) { all_nodes.insert(comp->node1_name); all_nodes.insert(comp->node2_name); }
    cout << "Available nodes:" << endl;
    if (all_nodes.empty()) { cout << "  (No nodes in circuit)" << endl; return; }
...
void Parser::handle_print(const vector<string>& args, Circuit& circuit) {
    ...
    if (analysis_type == "tran") {
        ...
        for (const auto& var : vars_to_print) cout << setw(15) << var;
        cout << endl;
...
int main() {
    Circuit circuit_main; Parser parser_main; string line_input;
    cout << "Welcome to SUTSpice! Enter commands." << endl;
    while (true) {
        cout << "> "; if (!getline(cin, line_input)) break;
